...
func TestAgentService_Get_NotFound(t *testing.T) {
	svc, _ := newTestService(t)
	_, apiErr := svc.GetAgent(context.Background(), uuid.New())
	require.NotNil(t, apiErr)
...
func TestAgentService_List_Pagination(t *testing.T) {
	...
	apiErr = svc.RetireAgent(ctx, retireTarget, false)
	require.Nil(t, apiErr)
...
func TestAgentService_Update_OptimisticConcurrency(t *testing.T) {
	...
	updated, apiErr := svc.UpdateAgent(ctx, created.ID, UpdateAgentRequest{
	...
	_, apiErr = svc.UpdateAgent(ctx, created.ID, UpdateAgentRequest{
		Role:    ptrStr("qa"),
		Version: ptrInt(created.Version),
	})
	require.NotNil(t, apiErr)
...
func TestAgentService_Update_RetiredAgent(t *testing.T) {
	...
	require.Nil(t, apiErr)
	apiErr = svc.RetireAgent(ctx, created.ID, false)
	require.Nil(t, apiErr)
...
func TestAgentService_Retire_NotFound(t *testing.T) {
	svc, _ := newTestService(t)
	apiErr := svc.RetireAgent(context.Background(), uuid.New(), false)
	require.NotNil(t, apiErr)
	assert.Equal(t, 404, apiErr.Status)
}
...
func TestAgentService_ListAgentCapabilities(t *testing.T) {
	...
	caps, apiErr := svc.ListAgentCapabilities(ctx, created.ID)
	require.Nil(t, apiErr)
	assert.Len(t, caps, 2)
