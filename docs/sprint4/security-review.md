...
| F-014 | Critical | AuthZ               | Cross-project task assignment (no `(task.project_id == agent.project_id == caller.project_id)` triple-check) | `src/internal/service/assignment.go` `AssignTaskToAgent` (no project check between lines 103 and 222) | Enforce the triple-check in the service layer (or in middleware). Data model §4.1 already plans an `assignments.project_id` column + backfill in Sprint 5; the runtime check should land with that migration. | **FIXED-IN-PATCH (TASK-420, 2026-06-13)**. Path-implied approach (Lead-accepted 2026-06-13; no `project_memberships` table yet — Sprint 5+ follow-up). `service/assignment.go` `AssignTaskToAgent` and `ListAssignmentHistory` gained a `callerProjectID uuid.UUID` parameter. `AssignTaskToAgent` enforces the triple-check (path-implied): `callerProjectID == uuid.Nil` → 400 `MISSING_PROJECT_HEADER`; `task.ProjectID != callerProjectID` → 404 `CROSS_TENANT_BLOCKED`; `agent.ProjectID != callerProjectID` → 404 `CROSS_TENANT_BLOCKED`; defensive `task.ProjectID != agent.ProjectID` → 404 `CROSS_TENANT_BLOCKED`. `ListAssignmentHistory` checks `task.ProjectID != callerProjectID` after fetch. `handler/assignment.go` `assignTask` and `ListHistory` extract `callerProjectID` via a local `projectIDFromContext(c)` helper (header `X-Project-ID`); empty/missing → 400 `MISSING_PROJECT_HEADER` without reaching the service. Two error helpers reused from TASK-419: `crossTenantBlocked()` and `missingProjectHeader()`. New tests: service-side `TestAssignTaskToAgent_CrossTenant_TaskInOtherProject`, `TestAssignTaskToAgent_CrossTenant_AgentInOtherProject`, `TestAssignTaskToAgent_CrossTenant_DefensiveTripleCheck`, `TestAssignTaskToAgent_MissingProjectHeader`, `TestListAssignmentHistory_CrossTenant`, `TestListAssignmentHistory_MissingProjectHeader`; handler-side `TestAssignmentHandler_Assign_CrossTenant`, `TestAssignmentHandler_Assign_MissingProjectHeader`, `TestAssignmentHandler_History_CrossTenant`, `TestAssignmentHandler_History_MissingProjectHeader`. Existing 25 `AssignTaskToAgent` calls and 3 `ListAssignmentHistory` calls in service test updated for the new param; mock interface and 14 handler tests updated. `[RUNTIME-FINDING]`. Confirms F-004. |
| F-015 | Critical | AuthZ / tenant scoping | Deliverable service has no project_id check; cross-tenant read/write via known UUID | `src/internal/handler/deliverable.go` `GetDeliverable` / `UpdateDeliverable` / `ListDeliverables` / `ListDeliverableVersions` / `CreateDeliverable`; `src/internal/service/deliverable.go` corresponding methods — all project_id-agnostic | Add a `project_id` filter to every read and to the `Create` validation (verify the deliverable's `task_id` and `agent_id` belong to a project the caller is a member of). | **FIXED-IN-PATCH (TASK-421, 2026-06-13)**. Path-implied approach (Lead-accepted 2026-06-13). `service/deliverable.go` all 5 methods (`CreateDeliverable`, `GetDeliverable`, `ListDeliverables`, `UpdateDeliverable`, `ListDeliverableVersions`) gained a `callerProjectID uuid.UUID` parameter as the LAST arg. `CreateDeliverable` enforces: `callerProjectID == uuid.Nil` → 400 `MISSING_PROJECT_HEADER`; `task.ProjectID != callerProjectID` → 404 `CROSS_TENANT_BLOCKED`; `agent.ProjectID != callerProjectID` → 404 `CROSS_TENANT_BLOCKED`; defensive `task.ProjectID != agent.ProjectID` → 404 `CROSS_TENANT_BLOCKED`. `GetDeliverable` / `UpdateDeliverable` / `ListDeliverableVersions` fetch the parent task and compare `task.ProjectID` to `callerProjectID` (deliverables have no `ProjectID` of their own; they inherit from the parent task). `ListDeliverables` does per-filter resolution: `TaskID` → task fetch + project compare; `AgentID` → agent fetch + project compare. `handler/deliverable.go` all 5 routes extract `callerProjectID` via a local `projectIDFromContext(c)` helper (header `X-Project-ID`); empty/missing → 400 `MISSING_PROJECT_HEADER` without reaching the service. Reused error helpers from TASK-419/420: `crossTenantBlocked()` and `missingProjectHeader()`. New tests: 11 service-side (`TestDeliverableService_Create_CrossTenant_TaskInOtherProject`, `..._Create_MissingProjectHeader`, `..._Get_CrossTenant`, `..._Get_MissingProjectHeader`, `..._List_CrossTenant_TaskFilterInOtherProject`, `..._List_CrossTenant_AgentFilterInOtherProject`, `..._List_MissingProjectHeader`, `..._Update_CrossTenant`, `..._Update_MissingProjectHeader`, `..._ListVersions_CrossTenant`, `..._ListVersions_MissingProjectHeader`); 11 handler-side matching. `seedDeliverableTaskAndAgent` test helper updated to take a `projectID` param and return it (matches TASK-420 `seedTaskAndAgent` pattern). `[RUNTIME-FINDING]`. |
...
- Sprint 4 patches reviewed (2026-06-12):
  ...
  - F-014: FIXED-IN-PATCH (TASK-420 — `callerProjectID` threaded through `AssignTaskToAgent` / `ListAssignmentHistory`; cross-tenant miss → 404 `CROSS_TENANT_BLOCKED`; missing header → 400 `MISSING_PROJECT_HEADER`). Updated 2026-06-13.
- F-013: FIXED-IN-PATCH (TASK-419 — `callerProjectID` threaded through `GetAgent` / `UpdateAgent` / `RetireAgent` / `ListAgentCapabilities`; cross-tenant miss → 404 `CROSS_TENANT_BLOCKED`; missing header → 400 `MISSING_PROJECT_HEADER`). Updated 2026-06-13.
  - F-015: FIXED-IN-PATCH (TASK-421 — `callerProjectID` threaded through all 5 `DeliverableService` methods; `CreateDeliverable` enforces task / agent / defensive triple-check on the parent task's `ProjectID`; `GetDeliverable` / `UpdateDeliverable` / `ListDeliverableVersions` fetch the parent task to compare project; `ListDeliverables` does per-filter resolution; cross-tenant miss → 404 `CROSS_TENANT_BLOCKED`; missing header → 400 `MISSING_PROJECT_HEADER`). 22 new tests (11 service + 11 handler). Updated 2026-06-13.
